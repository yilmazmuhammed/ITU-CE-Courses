...
void List::read_fromfile()
{
	...
	if ( !(phonebook = fopen( filename, "r+" ) ) ) {
		if ( !(phonebook = fopen( filename, "w+" ) ) ) {   // create an empty file and open it for write/update (both for input and output)
			...
			exit(1); // exit ends the program, the exitcode 1 is returned to the operating system, similar to returning a value to int main
		}
	}
...
{
	Phone_node *traverse, *tail, *newnode;
...
int List::search(char *target)
{
	...
	while (traverse) {
		...
		else if ( strnicmp(traverse->name, target, strlen(target)) == 0 ) { // if strings traverse->name and target are equal up to the length of target (case-insensitive) 
			found++;
			cout << counter << "." << traverse->name << " " << traverse->phonenum << endl;			
		}		
		traverse = traverse->next;		
